    ...
    <div className="bg-gray-900 py-24 sm:py-32">
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        <div className="mx-auto max-w-4xl text-center">
          <h2 className="text-base leading-7 font-semibold text-indigo-400">Pricing</h2>
        ...
        <div className="mx-auto mt-12 max-w-md space-y-8 sm:mt-16 lg:hidden">
          {tiers.map((tier) => (
            ...
            >
              <h3 id={tier.id} className="text-sm leading-6 font-semibold text-white">
                {tier.name}
              </h3>
              ...
              <a
                ...
                className={classNames(
                  ...
                  'mt-8 block rounded-md px-3 py-2 text-center text-sm leading-6 font-semibold focus-visible:outline-2 focus-visible:outline-offset-2'
                )}
              >
                Buy plan
              </a>
                        ...
                        return value ? (
                          <li key={feature.name} className="flex gap-x-3">
                            ...
                            <span>
                              {feature.name}{' '}
                              {typeof value === 'string' ? (
                                <span className="text-sm leading-6 text-gray-400">({value})</span>
                              ) : null}
                            </span>
                          </li>
                        ) : null
                ...
                <tr>
                  ...
                  {tiers.map((tier) => (
                    <th key={tier.id} scope="col" className="px-6 pt-6 xl:px-8 xl:pt-8">
                      <div className="text-sm leading-7 font-semibold text-white">{tier.name}</div>
                    </th>
                  ))}
                </tr>
              </thead>
              <tbody>
                  ...
                  {tiers.map((tier) => (
                    <td key={tier.id} className="px-6 pt-2 xl:px-8">
                      <div className="flex items-baseline gap-x-1 text-white">
                        ...
                        <span className="text-sm leading-6 font-semibold">/month</span>
                      </div>
                      <a
                        href={tier.href}
                        className={classNames(
                          ...
                          'mt-8 block rounded-md px-3 py-2 text-center text-sm leading-6 font-semibold text-white focus-visible:outline-2 focus-visible:outline-offset-2'
                        )}
                      >
                        Buy plan
                      </a>
                ...
                {sections.map((section, sectionIdx) => (
                  <Fragment key={section.name}>
                    <tr>
                      <th
                        ...
                        className={classNames(
                          ...
                          'pb-4 text-sm leading-6 font-semibold text-white relative'
                        )}
                      >
                        {section.name}
                    ...
                    {section.features.map((feature) => (
                      <tr key={feature.name}>
                        <th scope="row" className="py-4 text-sm leading-6 font-normal text-white relative">
                          {feature.name}
                        ...
                        {tiers.map((tier) => {
                          ...
                          return (
                            <td key={tier.id} className="px-6 py-4 xl:px-8">
                              {typeof value === 'string' ? (
                                <div className="text-center text-sm leading-6 text-gray-300">{value}</div>
                              ) : value === true ? (
                                <>
